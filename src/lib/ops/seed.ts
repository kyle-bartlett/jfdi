...
export function seedOpsIfEmpty() {
  ...
  const projects = [
    ...
    { title: 'JFDI', priority: 'P1', status: 'In Progress', description: 'Personal ops dashboard at jfdi.bartlettlabs.io — merged with Ops Dashboard. 14 pages: Dashboard, Reminders, Projects, Tasks, Goals, Calendar, Meetings, Relationships, Ops Center (9 sub-tabs), Action Queue, Automations, Spark, Knowledge, Search. 10 cron-built features awaiting Kyle review across 5 projects: C2 CPFR (jump-to-week, CSV export ⌘E), JFDI (quick-add, command palette ⌘K, reminders keyboard shortcuts), ZipWise (quick-complete stops), Gmail Brain (batch actions), KDP Book App (review queue shortcuts), Ops Dashboard (seed overhaul ×3). Comprehensive seed data: 30 projects, 13 prospects, 20+ agent tasks.' },
    ...
    { title: 'KDP Book Studio', priority: 'P2', status: 'Review', description: 'Kindle Direct Publishing automation at kdp-book-app.vercel.app. Kyle has 1 book on Amazon (coloring book). Cron-built: Review Queue keyboard shortcuts (a approve+auto-advance, h/l navigate, r reject, g regenerate, 1/2/3 switch views). Pending: Kyle review, CNAME books → Vercel DNS.' },
  ...
  const queueItems = [
    ...
    { queue_type: 'kyle', title: 'Review 10 cron-built features: C2 jump-to-week + CSV export (⌘E), JFDI quick-add + command palette (⌘K) + keyboard shortcuts, ZipWise quick-complete stops, Gmail Brain batch actions, KDP Book App review queue shortcuts, ops seed overhaul ×3', priority: 'P1', requested_by: 'Knox', status: 'Pending' },
  ...
  const agentTasks = [
    ...
    { agent_name: 'Stack', task_description: 'Gmail Brain Batch Actions — checkboxes + floating action bar (cron-built)', status: 'Completed', result_summary: 'Select all per section (Work/Personal), floating dark action bar, batch reclassify/undo, parallel processing in batches of 5. Commit 2f89f83, awaiting Kyle review.' },
    { agent_name: 'Stack', task_description: 'KDP Book App Review Queue Shortcuts — keyboard triage (cron-built)', status: 'Completed', result_summary: 'a approve+auto-advance, h/l navigate pages, r reject, g regenerate, 1/2/3 switch views, ? help. 50-page review: 150 clicks → 50 keypresses. Commit ca41443, awaiting Kyle review.' },
    { agent_name: 'Stack', task_description: 'Ops Dashboard seed comprehensive rewrite — 30 projects, 20 tasks, 20 events (cron-built)', status: 'Completed', result_summary: 'Full seed rewrite with real prospect data (Google ratings, mock sites), all 10 cron-built features logged, daily metrics, queue items. Commit 62cb3bc.' },
  ];
  ...
  const activityEvents = [
    ...
    { event_type: 'system', title: 'Autonomous cron dev: 15+ features shipped (2/19-2/20)', description: 'C2: discrepancy filter, jump-to-week, CSV export (⌘E). JFDI: quick-add, command palette (⌘K), reminders keyboard shortcuts. ZipWise: quick-complete stops. Gmail Brain: batch actions. KDP Book App: review queue shortcuts. Ops: seed data overhaul ×3. 10 features awaiting Kyle review, 2 deployed.', source: 'Cron', icon: '🔄' },
    ...
    { event_type: 'system', title: 'Gmail Brain Batch Actions (2/20, 7:22 AM)', description: 'Checkboxes, select all per section (Work/Personal), floating dark action bar, batch reclassify/undo, parallel processing. Commit 2f89f83.', source: 'Cron', icon: '📧' },
    { event_type: 'system', title: 'KDP Book App Review Queue Shortcuts (2/20, 9:22 AM)', description: 'a approve+auto-advance, h/l navigate, r reject, g regen, 1/2/3 switch views. 50 pages: 150 clicks → 50 keypresses. Commit ca41443.', source: 'Cron', icon: '📖' },
    { event_type: 'system', title: 'Ops Dashboard comprehensive rewrite (2/20, 9:58 AM)', description: '30 projects, 13 prospects (real local businesses), 20 agent tasks, 20 activity events, 13 queue items, daily metrics. Commit 62cb3bc.', source: 'Cron', icon: '📊' },
  ...
  db.prepare('INSERT INTO daily_metrics (id, metric_date, agents_deployed, tasks_completed, prospects_contacted, ideas_logged, active_streak, mood, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(uuid(), today, 0, 9, 0, 0, 2, 'grinding', 'Overnight+morning crons built 9 new features across 5 projects (C2 CSV export, JFDI keyboard shortcuts, ZipWise quick-complete stops, Gmail Brain batch actions, KDP Book App review queue shortcuts, ops seed overhaul ×3, ops ZipWise sync). 10 total features awaiting Kyle review. Lark Training deadline tomorrow (2/21). WoW Step 1b fired at 8:30 AM. Kyle offline 12+ hours — longest stretch recorded.');
